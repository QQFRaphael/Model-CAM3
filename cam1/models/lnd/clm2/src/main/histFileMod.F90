...
!-----------------------------------------------------------------------
    ...
    do f = 1,max_flds	
       fincl(f,1) = trim(hist_fincl1(f))
       fincl(f,2) = trim(hist_fincl2(f)) 
       fincl(f,3) = trim(hist_fincl3(f)) 
       fincl(f,4) = trim(hist_fincl4(f))	 
       fincl(f,5) = trim(hist_fincl5(f)) 
       fincl(f,6) = trim(hist_fincl6(f)) 

       fexcl(f,1) = trim(hist_fexcl1(f))
       fexcl(f,2) = trim(hist_fexcl2(f)) 
       fexcl(f,3) = trim(hist_fexcl3(f)) 
       fexcl(f,4) = trim(hist_fexcl4(f))	 
       fexcl(f,5) = trim(hist_fexcl5(f)) 
       fexcl(f,6) = trim(hist_fexcl6(f)) 
    end do
...
!-----------------------------------------------------------------------
    ...
    str = '$Name: cam3_1_brnchT_p2 $'
    call check_ret(&
         nf_put_att_text (nfid(t), NF_GLOBAL, 'version', len_trim(str), trim(str)), subname)

    str = '$Id: histFileMod.F90,v 1.19.6.22 2005/03/10 21:01:56 rosinski Exp $'
    call check_ret(&

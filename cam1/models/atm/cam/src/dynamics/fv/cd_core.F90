...
!BOC
! Local 2D arrays:
      real(r8)  wk(im+2,jfirst:jlast+2)
...
! fvitt
  real(r8)   u_cen(im,jfirst-ng_d:jlast+ng_d,kfirst:klast) ! cell centered u-Wind (m/s)
  real(r8)   v_cen(im,jfirst-ng_s:jlast+ng_d,kfirst:klast) ! cell centered v-Wind (m/s)
...
!
      ...
      if ( ( (ipe .eq. -1) .or. (everytime) ) .and. (.not. met_winds_on_walls()) ) then
         call get_met_fields( u_cen, v_cen )
         reset_winds = .true.
      else 
         reset_winds = .false.
      endif
...
!-----------------------------------------------------------------
         ...
         call c_sw( u(1,jfirst-ng_d,k),   v(1,jfirst-ng_s,k),         &
                    ...
                    u_cen(1,jfirst-ng_d,k), v_cen(1,jfirst-ng_s,k), reset_winds )
      enddo

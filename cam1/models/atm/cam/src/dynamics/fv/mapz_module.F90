...
! !LOCAL VARIABLES:
      ...
      real(r8)    pl, pr, qsum, qsumk(i1:i2), delp, esl
...
! Prepare contribution between pe1(i,ko(i,k)+1) and pe1(i,k0(i,k+1))
         ...
         do i = i1, i2
            do kl = k0(i,k)+1, k0(i,k+1)-1
               qsumk(i) = qsumk(i) + dp1(i,kl)*q4(1,i,kl)
            enddo
         enddo
...
! Check to see if pe2(i,k+1) and pe2(i,k) are in same pe1 interval
            if (k0(i,k+1) == k0(i,k)) then 
               ...
               q2(i,j,k) = q4(2,i,kk) + 0.5*(q4(4,i,kk)+q4(3,i,kk)-q4(2,i,kk))  &
                  *(pr+pl) - q4(4,i,kk)*r3*(pr*(pr+pl)+pl**2)
            else
! Consider contribution between pe2(i,k) and pe1(i,kk+1)
               qsum = (pe1(i,kk+1)-pe2(i,k))*(q4(2,i,kk)+0.5*(q4(4,i,kk)+       &
                  q4(3,i,kk)-q4(2,i,kk))*(1.+pl)-q4(4,i,kk)*                    &
                  (r3*(1.+pl*(1.+pl))))
...
! Now consider contribution between pe1(i,k0(i,k+1)) and pe2(i,k+1)
               ...
               esl = delp / dp1(i,kl)
               qsum = qsum + delp*(q4(2,i,kl)+0.5*esl*                          &
                  (q4(3,i,kl)-q4(2,i,kl)+q4(4,i,kl)*(1.-r23*esl)))
...
!
! !INTERFACE:
  subroutine mapn_ppm( km,   pe1,   q1,  nq, kn,   pe2,   q2,            &
                       ng_s, ng_n, itot, i1, i2,                         &
                       j, jfirst, jlast, iv, kord)
...
! Check to see if pe2(i,k+1) and pe2(i,k) are in same pe1 interval
            if (k0(i,k+1) == k0(i,k)) then 
               pr = (pe2(i,k+1)-pe1(i,kk)) / dp1(i,kk)
               q2(i,j,k,iq) = q4(2,i,kk) + 0.5*(q4(4,i,kk)+q4(3,i,kk)-q4(2,i,kk))  &
                  *(pr+pl) - q4(4,i,kk)*r3*(pr*(pr+pl)+pl**2)
            else
! Consider contribution between pe2(i,k) and pe1(i,kk+1)
               qsum = (pe1(i,kk+1)-pe2(i,k))*(q4(2,i,kk)+0.5*(q4(4,i,kk)+       &

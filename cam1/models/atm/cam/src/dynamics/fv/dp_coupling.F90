...
! !USES:
    ...
    use spmd_dyn,     only : ijk_xy_to_yz, local_dp_map, &
...
#if ( defined WACCM_GHG || defined WACCM_MOZART )
    call t_startf('DP_CPLN: ctem')
    call ctem_driver (u3,v3,omgaxy,pt(:,beglat:endlat,:),q3(:,beglat:endlat,:,1),ps,pe,beglat,endlat)
    call t_stopf('DP_CPLN: ctem')
#endif
...
! required q change in bottom level rescaled to mass fraction in 2nd level
                ...
                if (dqreq < qmavl) then
                   ...
                   if (dqreq>1.e-14 .and. iam .eq. 0) write (6,*) 'dpcoup dqreq', m, lchnk, i, qbot, qbotm1, dqreq
                else 
                   if (iam .eq. 0) write (6,*) 'dpcoup cant adjust', m, lchnk, i, qbot, qbotm1, dqreq
                end if
             end if
          end do
       end do
...
! !USES:
    ...
    use spmd_dyn, only : ikj_xy_to_yz, qxy_to_q,     &
                         ...
                         ijk_xy_to_yz, rxy_to_r, local_dp_map, &
                         block_buf_nrecs, chunk_buf_nrecs
    use mod_comm, only: mp_sendirr, mp_recvirr
#endif
...
! xy-decomposition instanciation immediately below
    ...
    real(r8), intent(out) :: ps(plon,beglat:endlat)

...
! !USES:
   ...
   use metdata,     only: met_ps_next, advance_met, get_us_vs
...
! Call the Lagrangian dynamical core using small tme step
   ...
   if( nq .ne. 0 ) then
      ...
      call adjust_press( im, jm, km, jfirst, jlast, &
                         kfirst, klast, ps, met_ps_next(1,jfirst), &
                         mfx, mfy, pexy, ifirstxy, ilastxy, jfirstxy, jlastxy )
      ...
      do i=1,im
         do j=jfirst,jlast
            do k=kfirst,klast
               if (i .ne. 1) cx(i,j,k) = mfx(i,j,k)/(.5*(dp0(i-1,j,k)+dp0(i,j,k)))
               if (i .eq. 1) cx(i,j,k) = mfx(i,j,k)/(.5*(dp0(1,j,k)+dp0(im,j,k)))
            enddo
         enddo
      enddo
      do i=1,im
         do j=jfirst,jlast
            do k=kfirst,klast
               if ((j .gt. 1) .and. (j .lt. jm)) cy(i,j,k) = mfy(i,j,k)/(.5*(dp0(i,j-1,k)+dp0(i,j,k)))/cose(j)
            enddo
         enddo
      enddo
...
! mass fluxes
      ...
      call trac2d( dp0(:,jfirst:jlast,:),    q3,     nc,     nq,     cx,             &

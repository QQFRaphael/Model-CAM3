...
!$omp parallel do private(j,i,jp), num_threads(ompinner)
      ...
      do j=js2g0,jn2g0
        do i=1,im
           jp = j-va(i,j)
           wk1v(i,j) = q(i,j) +0.5*va(i,j)*(q(i,jp)-q(i,jp+1))
        enddo
      enddo
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,iuw,iue,iu,itmp,isave,tmp,qmax,qmin,rut,ist,dm,a1,ar,a6), num_threads(ompinner)
#endif
  do j = jlow, jhigh
...
! Figure out ghost zone for the eastern edge:
      ...
      do i=1,im
! 2nd order slope
         tmp = 0.25*(qtmpv(i+1,j) - qtmpv(i-1,j))
      ...
      do i=1,im
         ...
         if(cv(i,j) .le. 0.) then
            itmp = i - iu
            isave(i) = itmp - 1
            fxv(i,j) = rut*(qtmpv(itmp,j)-dm(itmp)*(1.+rut))
         else
            itmp = i - iu - 1
            isave(i) = itmp + 1
            fxv(i,j) = rut*(qtmpv(itmp,j)+dm(itmp)*(1.-rut))
...
! Regular PPM (Eulerian without FFSL extension)
      ...
      if(iord == 1 .or. cosav(j) < cos_upw) then
         ...
         if( abs(iord).eq.2 .or. cosav(j) .lt. cos_van ) then
            do i=1,im
               iu = float(i) - cv(i,j)
               fxv(i,j) =  mfxv(i,j)*(qtmpv(iu,j)+dm(iu)*(sign(1.,cv(i,j))-cv(i,j)))
...
#ifdef  NEC_SX
   real (r8) qsum((-im/3)-1:im+im/3)   ! work arrays
#endif
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,ja), num_threads(ompinner)
#endif
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,ja,iu,itmp), num_threads(ompinner)
#endif
!dir$ concurrent
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,ja,tmp,qmax,qmin), num_threads(ompinner)
#endif
!dir$ concurrent
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,ja,iu,rut,itmp), num_threads(ompinner)
#endif
!dir$ concurrent
   ...
   do ja = 1, jatffn
      ...
      do i=1,im
        ...
        if(cv(i,j) .le. 0.) then
          itmp = i - iu
          isave(i,j) = itmp - 1
          fxv(i,j) = rut*(qtmpv(itmp,j)-dmv(itmp,j)*(1.+rut))
        else
          itmp = i - iu - 1
          isave(i,j) = itmp + 1
          fxv(i,j) = rut*(qtmpv(itmp,j)+dmv(itmp,j)*(1.-rut))
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,ja,ist), num_threads(ompinner)
#endif
!dir$ concurrent
...
! there is always at least one term from qsum (qtmpv) contributed to fxv.
!

      do i=1,im
        if(cv(i,j) >= 1.0 .and. (isave(i,j) < i) ) then
            fxv(i,j) = fxv(i,j) + (qsum(i-1) - qsum(isave(i,j) - 1))
        else if (cv(i,j) <= -1.0 .and. (i <= isave(i,j)) ) then
            fxv(i,j) = fxv(i,j) - (qsum(isave(i,j)) - qsum(i-1))
        end if
      end do
#endif
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,ja,iu), num_threads(ompinner)
#endif
!dir$ concurrent
   do ja = 1, jaftn
      j = jaft(ja)
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,ja,qmax,qmin), num_threads(ompinner)
#endif
!dir$ concurrent
...
! In-line xmist
      ...
      do i=1,im
         dmv(i,j) = r24*(8.*(qtmpv(i+1,j) - qtmpv(i-1,j)) + qtmpv(i-2,j) - qtmpv(i+2,j))
      enddo
...
! In-line xmist
      ...
      if(iord .le. 2) then
         ...
         do i=1,im
            dmv(i,j) = 0.25*(qtmpv(i+1,j) - qtmpv(i-1,j))
         enddo
      endif
...
!call ftrace_region_begin("xtpv_11")
   do ja = 1, jafftn2
      ...
      do i=1,im
         iu = float(i) - cv(i,j)
         fxv(i,j) =  mfxv(i,j)*(qtmpv(iu,j)+dmv(iu,j)*(sign(1.,cv(i,j))-cv(i,j)))
      enddo
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,jj), num_threads(ompinner)
#endif
!dir$ concurrent
 do jj = 1, jan
   j = ja(jj)
   ...
   do i=1,im
     al(i,j) = 0.5*(p(i-1,j)+p(i,j)) + (dm(i-1,j) - dm(i,j))*r3
   enddo
...
!dir$ concurrent
     do jj = 1, jan
       ...
       do i=1,im
         a6(i,j) = 3.*(p(i,j)+p(i,j)  - (al(i,j)+ar(i,j)))
       enddo
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,jj,iu,ru,itmp), num_threads(ompinner)
#endif
!dir$ concurrent
 do jj = 1, jbtn
   j = jbt(jj)
   ...
   do i=1,im
     ...
     if(c(i,j) .gt. 0.) then
       itmp = i - iu - 1
       isave(i,j) = itmp + 1
       fx(i,j) = ru*(ar(itmp,j)+0.5*ru*(al(itmp,j)-ar(itmp,j) +     &
                 a6(itmp,j)*(1.-r23*ru)) )
     else
       itmp = i - iu
       isave(i,j) = itmp - 1
       fx(i,j) = ru*(al(itmp,j)-0.5*ru*(ar(itmp,j)-al(itmp,j) +     &
                 a6(itmp,j)*(1.+r23*ru)) )
     endif
   enddo
...
!dir$ concurrent
 do jj = 1, jbfn
   ...
   do i=1,im
     if(c(i,j) .gt. 0.) then
       fx(i,j) = ar(i-1,j) + 0.5*c(i,j)*(al(i-1,j) - ar(i-1,j) +   &
                 a6(i-1,j)*(1.-r23*c(i,j)) )
     else
       fx(i,j) = al(i,j) - 0.5*c(i,j)*(ar(i,j) - al(i,j) +         &
                 a6(i,j)*(1.+r23*c(i,j)))
     endif
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,jj,xxx,bbb,ccc), num_threads(ompinner)
#endif
!dir$ concurrent
 do jj = 1, jan
   j = ja(jj)
...
! needs p(-1:im+2,j), d2(0:im+1,j)
   do i=1,im
      if( d2(i+1,j)*d2(i-1,j).lt.0. .and. p(i+1,j).ne.p(i-1,j) ) then
          xxx    = 1. - 0.5 * ( p(i+2,j) - p(i-2,j) ) / ( p(i+1,j) - p(i-1,j) )
          eta(i,j) = max(0., min(xxx, 0.5) )
      else
          eta(i,j) = 0.
      endif
   enddo
...
! needs eta(0:im,j), dh(0:im-1,j), dm(0:im,j)
   do i=1,im
      bbb = ( 2.*eta(i,j  ) - eta(i-1,j) ) * dm(i-1,j) 
      ccc = ( 2.*eta(i-1,j) - eta(i,j  ) ) * dm(i,j  ) 
      al(i,j) = al(i,j) + 0.5*( eta(i-1,j) - eta(i,j)) * dh(i-1,j) + (bbb - ccc) * r3
   enddo
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,jj,pmp.lac,pmin,pmax), num_threads(ompinner)
#endif
!dir$ concurrent
   do jj = 1, jan
      j = ja(jj)
...
! Constraint for AR
!     i = 1
      pmp   = p(1,j) + 2.0 * d1(1,j)
      lac   = p(1,j) + 0.5 * (d1(1,j)+d2(im,j)) + d2(im,j) 
      pmin  = min(p(1,j), pmp, lac)
      pmax  = max(p(1,j), pmp, lac)
      ar(1,j) = min(pmax, max(ar(1,j), pmin))

      do i=2, im
         pmp   = p(i,j) + 2.0*d1(i,j)
         lac   = p(i,j) + 0.5*(d1(i,j)+d2(i-1,j)) + d2(i-1,j)
...
! Constraint for AL
      do i=1, im-1
         pmp   = p(i,j) - 2.0*d1(i+1,j)
         lac   = p(i,j) + 0.5*(d2(i+1,j)-d1(i+1,j)) + d2(i+1,j)
         pmin  = min(p(i,j), pmp, lac)
         pmax  = max(p(i,j), pmp, lac)
...
! i=im
      i = im
      pmp    = p(im,j) - 2.0*d1(1,j)
      lac    = p(im,j) + 0.5*(d2(1,j)-d1(1,j)) + d2(1,j)
...
! compute A6 (d2)
      do i=1, im
         d2(i,j) = 3.*(p(i,j)+p(i,j)  - (al(i,j)+ar(i,j)))
      enddo
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,jj,da1,da2,a6da), num_threads(ompinner)
#endif
!dir$ concurrent
    do jj = 1, jan
      j = ja(jj)
        ...
        do i=1,im
           if(dm(i,j) .eq. 0.) then
               ...
               a6da = a6(i,j)*da1
               if(a6da .lt. -da2) then
                  a6(i,j) = 3.*(al(i,j)-p(i,j))
                  ar(i,j) = al(i,j) - a6(i,j)
               elseif(a6da .gt. da2) then
                  a6(i,j) = 3.*(ar(i,j)-p(i,j))
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,jj,da1,dl,dr), num_threads(ompinner)
#endif
!dir$ concurrent
    do jj = 1, jan
      j = ja(jj)
...
#if defined( NESTED_PAR )
!$omp parallel do private(j,i,jj,fmin), num_threads(ompinner)
#endif
!dir$ concurrent
    do jj = 1, jan
      j = ja(jj)
      ...
      do i=1,im
        if(abs(ar(i,j)-al(i,j)) .lt. -a6(i,j)) then
          fmin = p(i,j) + 0.25*(ar(i,j)-al(i,j))**2/a6(i,j) + a6(i,j)*r12
          if(fmin.lt.0.) then

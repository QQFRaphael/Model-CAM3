...
!-----------------------------------------
! fvitt
! make sure top interface is non zero
      if (hyai(1) .eq. 0.) then
         if (hybm(1) .ne. 0.0) then
            hyai(1) = hybm(1)*1.e-2
         else if (hyam(1) .ne. 0.0) then
            hyai(1) = hyam(1)*1.e-2
         else
      ...
      ptop = ak(1)
      if ( iam == 0 ) then
         write(6,*) 'Using hyai & hybi from IC:', 'KS=',ks,' PTOP=',ptop
      endif
   endif
...
!----------------------------------------------------------
   ...
   write (6,*) "STEPON: pre-processor variable STAGGERED must be set"
   write (6,*) "in you misc.h file. Enter: #define STAGGERED"
   write (6,*) "Then recompile CAM. Quitting."
   call endrun
#endif
...
!
      call diag_dynvar_ic (phis, ps, t3, u3s, v3s, q3 )
!
!--------------------------------------------------------------------------

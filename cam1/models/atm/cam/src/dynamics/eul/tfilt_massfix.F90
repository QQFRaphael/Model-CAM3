...
!
   ...
   real(r8), intent(inout) :: qm2(plon,plev,pcnst+pnats)
...
! Input/Output arguments
   ...
   real(r8), intent(inout) :: q3(plon,plev,pcnst+pnats)
   ...
   real(r8), intent(inout) :: q3m1(plon,plev,pcnst+pnats)
!
! Local workspace
!
...
!   real(r8) engp                   ! Potential energy integral
   ...
   real(r8) :: q3forecast(plon,plev,ppcnst),delta_q3(plon,plev,ppcnst)
   real(r8) divt3d(plon,plev)      ! 1./pmid
   real(r8) divq3d(plon,plev,ppcnst)      ! 1./pmid
...
!
   ...
   do i=1,nlon
      do m=1,ppcnst
         do k=1,plev
            divq3d(i,k,m)= (qfcst(i,k,m)-qminus(i,k,m))/ztodt
...
! timestep
!
   call outfld('t',t3sav(1,1,lat)    ,plon   ,lat     )
   call outfld('q',q3sav(1,1,1,lat),plon   ,lat     )
   call outfld('Ps',pssav(1,lat)    ,plon   ,lat     )
   call outfld('u',u3sav(1,1,lat)    ,plon   ,lat     )
   call outfld('v',v3sav(1,1,lat)    ,plon   ,lat     )
!
...
!
   fixmas_plon(:)=fixmassav(lat)
   beta_plon(:)=betasav(lat)
   clat_plon=clat(lat)
   ...
   call outfld('divq3d',divq3dsav(1,1,1,lat)  ,plon   ,lat     )
   call cnst_get_ind('CLDLIQ', ixcldliq)
   call cnst_get_ind('CLDICE', ixcldice)

   if (nstep.eq.0) then
     call outfld('dcldliq',q3sav(1,1,ixcldliq,lat),plon   ,lat     )
     call outfld('dcldice',q3sav(1,1,ixcldice,lat),plon   ,lat     )
   else
     call outfld('dcldliq',divq3dsav(1,1,ixcldliq,lat),plon   ,lat     )
     call outfld('dcldice',divq3dsav(1,1,ixcldice,lat),plon   ,lat     )
   endif 
!
...
#if ( defined BFB_CAM_SCAM_IOP )
   do m=1,pcnst
      alpha_plon(:,m)= alphasav(m,lat)
      call outfld(alphanam(m),alpha_plon(1,m) ,plon   ,lat     )
      call outfld(dqfxnam(m),dqfx3savm1(1,1,m,lat) ,plon   ,lat     )
      alphasav(m,lat)= alpha(m)
      dqfx3savm1(:nlon,:,m,lat)= dqfx3sav(:nlon,:,m)
   end do
#endif
#endif
!
...
!
   ...
   if (nstep.ge.2) then
      ...
      do k=1,plev
         ...
         do m=2,pcnst+pnats
            if (cnst_get_type_byind(m) .eq. 'wet') then !else: see cnst_need_pdeldry logic, below
               do i=1,nlon
         ...
         if ( cnst_need_pdeldry) then 
            do m=2,pcnst+pnats
               if (cnst_get_type_byind(m) .eq. 'dry') then !else: see above cnst_need_pdeldry logic

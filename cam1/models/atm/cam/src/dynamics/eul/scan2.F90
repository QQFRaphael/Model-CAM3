...
#else
subroutine scan2run (ztodt, cwava, etamid)
#endif
!----------------------------------------------------------------------- 
! 
! Purpose: 
...
!
   integer m,n                         ! indices
   integer lat,j,irow,jdry                 ! latitude indices
...
!$OMP PARALLEL DO PRIVATE (LAT)
   ...
   do lat=beglat,endlat
      ...
      call tfilt_massfixrun (ztodt,       lat, u3(1,1,lat,n3m1),u3(1,1,lat,n3), &
                          ...
                          pdeld(:,:,jdry,n3), pdeld(:,:,jdry,n3m1), pdeld(:,:,jdry,n3m2))
...
!
!-----------------------------------------------------------------------
   use pmgrid, only: iam, numlats, plat
   use mpishorthand, only: mpicom, mpir8
   use spmd_dyn
!---------------------------------Parameters----------------------------------

...
!
         tmp1 = 0.5*(phis_tmp(i,latm) - phis_tmp(i,latp))
         tmp2 = 0.5*(phis_tmp(i,latm) + phis_tmp(i,latp))
...
! Compute symmetric and antisymmetric components
!
         tmp1 = 0.5*(log_ps(i,latm) - log_ps(i,latp))
         tmp2 = 0.5*(log_ps(i,latm) + log_ps(i,latp))
         log_ps(i,latm) = tmp1
         log_ps(i,latp) = tmp2
...
!
         do i=1,2*nmmax(irow)
            tmp1 = 0.5*(x3_tmp(i,k,latm) - x3_tmp(i,k,latp))
            tmp2 = 0.5*(x3_tmp(i,k,latm) + x3_tmp(i,k,latp))
...
!
      do irow=1,plat/2
         ...
         do i=1,2*nmmax(irow)
            ...
            tmp1 = 0.5*(u_cosphi(i,k,latm) - u_cosphi(i,k,latp))
            tmp2 = 0.5*(u_cosphi(i,k,latm) + u_cosphi(i,k,latp))
            u_cosphi(i,k,latm) = tmp1
            u_cosphi(i,k,latp) = tmp2

            tmp1 = 0.5*(v_cosphi(i,k,latm) - v_cosphi(i,k,latp))
            tmp2 = 0.5*(v_cosphi(i,k,latm) + v_cosphi(i,k,latp))

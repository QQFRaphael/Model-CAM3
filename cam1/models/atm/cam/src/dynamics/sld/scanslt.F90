...
!-----------------------------------------------------------------------
!
! $Id: scanslt.F90,v 1.14.2.10 2005/03/02 20:52:46 bundy Exp $
!
!-----------------------------------------------------------------------
...
!
  ...
  real(r8), parameter :: epssld = 0.2     ! "epsilon" for SLD decentering algorithm
...
!
  if (masterproc) then
     ...
     write(6,*)'Node  Extended Partition'
     write(6,*)'-----------------------------------------'
     do procid=0,npes-1
        write(6,200) procid,cutex(1,procid), cutex(2,procid)
...
!-----------------------------------------------------------------------
    ...
    if ( (beglatex == 0) .or. (endlatex == 0) ) then
       call endrun(sub//': extended grid begining and ending latitude have NOT been set yet!')
    end if
...
!
      do k = 1, plevp
         do i = 1, nlon(j)
            icen = i1 + i - 1
            adv_state%etadot(icen,k,jcen) = 2.*etadot(i,k,j) - &
...
!$OMP PARALLEL DO PRIVATE (I,K,J,JCEN,ICEN)
   do j = beglat, endlat
      ...
      do k = 1, plev
         do i = 1, nlon(j)
            icen = i1 + i - 1
            adv_state%u3sld(icen,k,jcen) = 2.*u3(i,k,j,n3) - u3(i,k,j,n3m1)
            adv_state%v3sld(icen,k,jcen) = 2.*v3(i,k,j,n3) - v3(i,k,j,n3m1)
         end do
      end do
   end do
...
!$OMP PARALLEL DO PRIVATE (LAT,WORK)
   do lat=beglat,endlat
      fftbuf(nlon(lat)+1:nlon_fft,:,:,lat) = 0.0
...
!
        ump(i,k) = 0.5*(u3sld(ii,k,jcen) + u3p(i,k))*ra
        vmp(i,k) = 0.5*(v3sld(ii,k,jcen) + v3p(i,k))*ra
...
!
     do k = 1,plev
        do i = 1,nlon
           ii = i + i1 - 1
           ump(i,k) = 0.5*(ump(i,k) + u3p(i,k))
           vmp(i,k) = 0.5*(vmp(i,k) + v3p(i,k))
           wmp(i,k) = 0.5*(wmp(i,k) + wmpa(i,k)       )
        end do
     end do
!

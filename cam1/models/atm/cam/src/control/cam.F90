...
!-----------------------------NOTICE------------------------------------
!
!            Community Atmospheric Model, $Name: cam3_1_brnchT_p2 $ (CAM)
!
! 
...
!
   if (gptlsetoption (gptlcpu, 0) < 0) call endrun ('CAM: gptlsetoption')
!
! Compile-time setting of max timer depth
!
#ifdef GPTLDEPTHLIMIT
   if (gptlsetoption (gptldepthlimit, GPTLDEPTHLIMIT) < 0) call endrun ('CAM: gptlsetoption')
#endif
!
...
!
!   if (gptlsetoption (PAPI_TOT_CYC, 1) < 0) call endrun ('CAM: gptlsetoption')
...
!
   if (gptlinitialize () < 0) call endrun ('CAM: gptlinitialize')
...
!
   if (masterproc) then
      ...
      write(6,*)'$Name: cam3_1_brnchT_p2 $ '
      write(6,*)'$Date: 2005/03/10 21:00:24 $'
      write(6,*)'------------------------------------------------------------'
      write(6,*)'(Online documentation is available on the CAM'
      write(6,*)' home page: http://www.ccsm.ucar.edu/models/atm-cam/'
      write(6,*)' License information is available as a link from above'
      write(6,*)'------------------------------------------------------------'
!      write(6,*)'WARNING - output from all but master task suppressed' 
!      write(6,*)'------------------------------------------------------------'
!   else

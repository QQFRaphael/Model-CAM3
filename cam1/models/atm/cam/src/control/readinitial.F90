...
!
      if (nf_inq_varid (ncid, 'nlon', nlonid)  ==  nf_noerr) then
         ...
         do j = 1,plat
            if(nlon(j) > plon) then
               write(6,*)'READINITIAL Error: nlon read from IC file is greater than plon at latitude ',j
               write(6,*)'Model Parameter:   plon: = ',plon
               write(6,*)'Dataset Parameter: nlon = ',nlon(j)
               call endrun
            end if
            if (nlon(j) /= nlon(plat-j+1)) then
               write(6,*)'READINITIAL: nlon not symmetric at j=', j
               call endrun
            end if
            if(wnummax(j) > ptrm) then
               write(6,*)'READINITIAL Error: wnummax read from IC file is greater than ptrm at latitude ',j
               write(6,*)'Model Parameter:    ptrm    = ',ptrm
               write(6,*)'Dataset Parameter:  wnummax = ',wnummax(j)
               call endrun
            end if
         end do
      else

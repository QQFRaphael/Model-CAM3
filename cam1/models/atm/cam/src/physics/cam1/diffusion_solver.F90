...
contains
  ...
  subroutine compute_vdiff ( lchnk, &
    ...
    real(r8), intent(inout) :: kvq(pcols,pver+1)      ! diffusivity for constituents
    real(r8), intent(inout) :: cgs(pcols,pver+1)      ! countergradient star (cg/flux)
    real(r8), intent(inout) :: cgh(pcols, pver+1)     ! countergradient term for heat
...
!   integer  :: nbot_molec                         ! bottom level where molecular diffusivity is applied
    ...
    if ( do_molec_diff ) then
       ...
       !     cnst_fixed_ubc , mw_fac , ntop_molec , and ubc_t

...
!------------------------------------------------------------------------------------------------
! Purpose:
! Provides distributions of CH4, N2O, CFC11, and CFC12 using a simple
! prognostic scheme with chemical loss frequencies from Portmann and Solomon.
! The concentrations in the bottome two levels are maintained at the values
! set by the chem_surfvals module.
!
! Revision history:
!             B. Boville      Original version
! 2004-08-24  B. Eaton        Generalize interface so all constituent concentration data
!                             is supplied to the radiation routines via chem_get_cnst.
! 2004-08-30  B. Eaton        Move generalized radiation/chemistry interface into its
!                             own module.  Remove chem_get_cnst method since prognostic
!                             constituents are accessed directly from the constituent array.
!------------------------------------------------------------------------------------------------
  ...
  use physconst,      only: mwdry, mwch4, mwn2o, mwf11, mwf12, mwh2o, mwo3, gravit
...
!
  ...
  public chem_defaultopts               ! set default values of namelist variables
  public chem_setopts                   ! get namelist input
  public chem_is_active                 ! returns true if this package is active (trace_gas=.true.)
...
! Private data
  ...
  logical            :: trace_gas = .false.  ! .true. => this package is active
...
! Time/space interpolation of loss frequencies
  ...
  character(len=8), dimension(ncnst), parameter :: c_names = & ! constituent names
     (/'N2O  ', 'CH4  ', 'CFC11', 'CFC12'/)
  character(len=13), dimension(ncnst), parameter :: c_long = & ! constituent longnames
     (/'Nitrous Oxide', 'Methane      ', 'CFC11        ', 'CFC12        '/)
  real(r8), dimension(ncnst), parameter :: c_mw = &  ! constituent molecular weights
     (/mwn2o, mwch4, mwf11, mwf12/)
  real(r8), dimension(ncnst), parameter :: c_cp = &  ! constituent heat capacities
     (/666._r8, 666._r8, 666._r8, 666._r8/)
  real(r8), dimension(ncnst), parameter :: c_qmin = &! constituent minimum mmr
     (/0._r8, 0._r8, 0._r8, 0._r8/)
  integer, dimension(ncnst) :: c_i                   ! global constituent index
...
!-----------------------------------------------------------------------
   ...
   if (.not. trace_gas) return

   do i = 1, ncnst
      call cnst_add(c_names(i), advected, c_mw(i), c_cp(i), c_qmin(i), c_i(i), longname=c_long(i))
   end do
   ixghg = c_i(1)
...
subroutine chem_defaultopts( &
   trace_gas_out, bndtvg_out, h2orates_out, chem_config_out, airpl_emis_file_out, &
   nox_emis_file_out, co_emis_file_out, ch2o_emis_file_out, sad_scenario_out, sad_fixed_date_out, &
   sad_file_out, sulf_file_out, depvel_file_out, n2d_file_out, xs_coef_file_out, &
   xs_short_file_out, xs_long_file_out, rsf_file_out)
!----------------------------------------------------------------------- 
! Purpose: Return default runtime options
!-----------------------------------------------------------------------

   logical,          intent(out), optional :: trace_gas_out
   character(len=*), intent(out), optional :: bndtvg_out
   character(len=*), intent(out), optional :: h2orates_out
   character(len=*), intent(out), optional :: chem_config_out
   character(len=*), intent(out), optional :: airpl_emis_file_out
   character(len=*), intent(out), optional :: nox_emis_file_out
   character(len=*), intent(out), optional :: co_emis_file_out
   character(len=*), intent(out), optional :: ch2o_emis_file_out
   character(len=*), intent(out), optional :: sad_scenario_out
   integer,          intent(out), optional :: sad_Fixed_date_out
   character(len=*), intent(out), optional :: sad_file_out
   character(len=*), intent(out), optional :: sulf_file_out
   character(len=*), intent(out), optional :: depvel_file_out
   character(len=*), intent(out), optional :: n2d_file_out
   character(len=*), intent(out), optional :: xs_coef_file_out
   character(len=*), intent(out), optional :: xs_short_file_out
   character(len=*), intent(out), optional :: xs_long_file_out
   character(len=*), intent(out), optional :: rsf_file_out
!-----------------------------------------------------------------------

   if ( present(trace_gas_out) ) then
      trace_gas_out = trace_gas
   endif
   if ( present(bndtvg_out) ) then
      bndtvg_out = bndtvg
   endif

end subroutine chem_defaultopts

!================================================================================================

subroutine chem_setopts( &
   trace_gas_in, bndtvg_in, h2orates_in, chem_config_in, airpl_emis_file_in, &
   nox_emis_file_in, co_emis_file_in, ch2o_emis_file_in, sad_scenario_in, sad_fixed_date_in, &
   sad_file_in, sulf_file_in, depvel_file_in, n2d_file_in, xs_coef_file_in, &
   xs_short_file_in, xs_long_file_in, rsf_file_in)

!----------------------------------------------------------------------- 
! Purpose: Set runtime options
!-----------------------------------------------------------------------

   logical,          intent(in), optional :: trace_gas_in
   character(len=*), intent(in), optional :: bndtvg_in
   character(len=*), intent(in), optional :: h2orates_in
   character(len=*), intent(in), optional :: chem_config_in
   character(len=*), intent(in), optional :: airpl_emis_file_in
   character(len=*), intent(in), optional :: nox_emis_file_in
   character(len=*), intent(in), optional :: co_emis_file_in
   character(len=*), intent(in), optional :: ch2o_emis_file_in
   character(len=*), intent(in), optional :: sad_scenario_in
   integer,          intent(in), optional :: sad_Fixed_date_in
   character(len=*), intent(in), optional :: sad_file_in
   character(len=*), intent(in), optional :: sulf_file_in
   character(len=*), intent(in), optional :: depvel_file_in
   character(len=*), intent(in), optional :: n2d_file_in
   character(len=*), intent(in), optional :: xs_coef_file_in
   character(len=*), intent(in), optional :: xs_short_file_in
   character(len=*), intent(in), optional :: xs_long_file_in
   character(len=*), intent(in), optional :: rsf_file_in
!-----------------------------------------------------------------------

   if ( present(trace_gas_in) ) then
      trace_gas = trace_gas_in
   end if
   if ( present(bndtvg_in) ) then
      bndtvg = bndtvg_in
   endif

end subroutine chem_setopts
...
!-----------------------------------------------------------------------
    ...
    if (trace_gas) then
       ...
       ! Set names of constituent sources and declare them as history variables
       do m = 1, ncnst
          mm = ixghg-1+m
          srcnam(m) = trim(cnst_name(mm)) // 'SRC'

          call addfld (cnst_name(mm), 'kg/kg   ', pver, 'A', cnst_longname(mm), phys_decomp)
          call addfld (srcnam(m),     'kg/kg/s ', pver, 'A', trim(cnst_name(mm))//' source/sink', phys_decomp)
...
!===============================================================================
  ...
  subroutine chem_timestep_tend (state, ptend, cflx, dt, pbuf, fh2o)
!----------------------------------------------------------------------- 
! 
! Purpose: 
...
!
    ...
    real(r8) ch4vmr                               ! methane vmr at surface
    real(r8) n2ovmr                               ! nitrous oxide vmr at surface
    real(r8) f11vmr                               ! cfc11 vmr at surface
    real(r8) f12vmr                               ! cfc12 vmr at surface
!
!-----------------------------------------------------------------------
!
...
!
    n2ovmr = chem_surfvals_get('N2OVMR')
    ch4vmr = chem_surfvals_get('CH4VMR')
    f11vmr = chem_surfvals_get('F11VMR')
    f12vmr = chem_surfvals_get('F12VMR')
    do k = pver-1, pver
       do i=1,ncol

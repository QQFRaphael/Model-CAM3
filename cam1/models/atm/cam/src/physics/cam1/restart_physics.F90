...
!
	...
	tmpfield(:,i) = srfflx_state2d(i)%lwup(:)
      ...
      call write_field_from_chunk(nrg,1,1,1,sgh30)
      do i=begchunk,endchunk
	...
	tmpfield(:,i) = srfflx_state2d(i)%sst(:)
      ...
      do i=begchunk,endchunk
        ncol = get_ncols_p(i)
        tmpfield3d(:ncol,:,i) = surface_state2d(i)%tssub(:ncol,:)
      end do
...
!
      if (aeres) then
         ...
         call write_field_from_chunk(nrg2, 1, (pverp-ntoplw+1)*(pverp-ntoplw+1), 1, abstot_3d(1,ntoplw,ntoplw,begchunk))
...
!#######################################################################
   ...
   subroutine read_restart_physics (nrg, nrg2 )
...
!
	...
	srfflx_state2d(i)%lwup(:ncol) = tmpfield(:ncol,i)
      ...
      call read_chunk_from_field(nrg,1,1,1,sgh30)
      call read_chunk_from_field(nrg,1,1,1,tmpfield)
      do i=begchunk,endchunk
        ncol = get_ncols_p(i)
...
!
      ...
      if (aeres) then
         ...
         call read_chunk_from_field(nrg2, 1, (pverp-ntoplw+1)*(pverp-ntoplw+1), 1, abstot_3d(1,ntoplw,ntoplw,begchunk))

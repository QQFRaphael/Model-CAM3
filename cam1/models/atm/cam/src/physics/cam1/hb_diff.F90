...
module hb_diff
  ...
  use pmgrid, only    : masterproc          ! output from hb_init should be eliminated 
  use constituents, only: pcnst, pnats      ! these should be passed in
...
CONTAINS
  ...
  subroutine init_hb_diff(gravx,      cpairx, rairx, zvirx, ntop_eddy,  &
    ...
    if (masterproc) then
       write(6,*)'INIT_HB_DIFF: PBL height will be limited to bottom ',npbl, &

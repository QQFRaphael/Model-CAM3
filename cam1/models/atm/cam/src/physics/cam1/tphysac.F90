...
!-----------------------------------------------------------------------
   ...
   use ion_drag_calc,      only: iondrag
#endif
...
!
   ...
   real(r8) :: kvh(pcols,pver+1)                   ! diffusion coefficient for heat
...
! Test tracers
   ...
   if (chem_is_active()) then
      call chem_timestep_tend(state, ptend, cflx, ztodt, pbuf, fh2o)
      call physics_update (state, tend, ptend, ztodt)
...
!===================================================
   ...
   call vertical_diffusion_tend (ztodt ,state ,taux ,tauy ,shf     ,&
                 cflx     ,pblh     ,tpert    ,qpert    , surfric  ,&
                 obklen   ,ptend    ,cld      ,ocnfrac  , landfrac ,&
                 sgh30    ,pbuf     ,kvh)
...
! Ion drag calculation
   call iondrag (lchnk,ncol,state,ptend)
   call physics_update (state, tend, ptend, ztodt)
! Check energy integrals

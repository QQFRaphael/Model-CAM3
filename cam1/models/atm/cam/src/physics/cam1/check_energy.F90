...
!-----------------------------------------------------------------------
    ...
    if (print_energy_errors) then
       if (any(abs(te_rer(1:ncol)) > 1.E-14 .or. abs(tw_rer(1:ncol)) > 1.E-10)) then
          do i = 1, ncol
...
!!$    print *, 'heat_glob',heat_glob
    if (masterproc) then
       write (6,*) "nstep, te", nstep, teinp_glob, tedif_glob/dtime, heat_glob, psurf_glob
    end if

...
#
#	$Id: CAM_run.pm,v 1.15.4.20 2004/09/23 17:14:19 erik Exp $
#
...
#
use Env qw(LOGNAME F_OPTIMIZATION_OVERRIDE);
use CAM;
use CAM_namelist;
use CAM_lab;
use English;
...
#
  ...
  #
  my @list = ("INC_MPI", "INC_NETCDF", "LIB_MPI", "LIB_NETCDF" );
  foreach my $key ( @list ) {
    if ( ! defined($self->CFG->cfg( $key ) ) ) {
...
#---------------------------------------------------------------------------------------
                  if ( $SPMD eq "TRUE" ) {
                    my %env = ( MP_EUILIB=>"us", MP_RMPOOL=>"1" );
                    foreach my $key ( keys(%env) ) {
                      if ( $ENV{$key} !~ /.+/ ) { 
                         $$run_ref{$key} = $env{$key};
                      } else {
                  ...
                  if ( $SMP eq "TRUE" ) {
                    my %env = ( XLSMPOPTS=>"stack=40000000" );
                    foreach my $key ( keys(%env) ) {
                      if ( $ENV{$key} !~ /.+/ ) { 
                         $$run_ref{$key} = $env{$key};
                      } else {
...
#---------------------------------------------------------------------------------------
  ...
  if ( $SMP eq "TRUE" ) {
    ...
    # If SHMEM_CPUS set to zero, set to undefined (so OS default will be used on this machine)
    if ( $self->{SHMEM_CPUS} == 0 ) { 
      $self->{SHMEM_CPUS} = undef;
    }
    if ( defined($self->{SHMEM_CPUS}) ) { 
...
#
  ...
  my %opt = ( PCNST=>'-nadv', PNATS=>'-nnadv', RESOLUTION=>'-res',
                 ...
                 MODEL_MODDIR=>'-usr_src', PCOLS=>'-pcols',
  ...
  if ( defined($self->env("PRINTLEV")) && ($self->env("PRINTLEV") == 0) ) {
    $options = $options . " -s";
  }
  if ( defined($ENV{F_OPTIMIZATION_OVERRIDE}) ) {
      $options = $options . " -fopt $ENV{F_OPTIMIZATION_OVERRIDE}";
  }
  return( $options );
}
...
#
  ...
  if ( ! defined($self->env("SILENT")) || ($self->env("SILENT") =~ /FALSE/) ) {
...
#
  ...
  if ( defined($self->env("SILENT")) && ($self->env("SILENT") =~ /TRUE/) ) {

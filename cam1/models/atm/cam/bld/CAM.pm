...
#
#	$Id: CAM.pm,v 1.17.4.14 2004/06/09 16:03:14 erik Exp $
#
...
#
  ...
  my @env_list = (
          ...
          "NAMELIST_TO_READ"       # Namelist to read
  );
...
#
  ...
  if ( defined($input) && ($input ne "alldata") ) {
    ...
    $self->chdir( $input );
    $self->exec( "/bin/rm -rf *.[do] *.mod esmf Depends Srcfiles *.stb" );
...
#
  ...
  if ( $? != 0 ) {
    ...
    $self->die( "ERROR:: Trouble executing: $command: Return status = ", $?/512 );
    return;
  }
}
...
# in the CAMEXP and CLMEXP associative arrays
#
  my $self = shift;
  my $CAMEXPref = shift;
  my $CLMEXPref = shift;
  ...
  if ( $self->do_build_namelist ) {
    ...
    $opts{'test'}        = 1;
    $opts{'namelist'}    = $self->env("NAMELIST_TO_READ");
    $opts{'ncdata_vers'} = $self->env( "NCDATA_VERS" );
    $opts{'interactive'} = 0;
    if ( defined($self->env("PRINTLEV")) ) {
      $opts{'printlev'} = $self->env("PRINTLEV");
    } else {
      $opts{'printlev'}    = 1;
    }
    #

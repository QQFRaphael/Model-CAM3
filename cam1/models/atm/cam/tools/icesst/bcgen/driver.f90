...
!
   namelist /cntlvars/ mon1, iyr1, monn, iyrn, mon1rd, iyr1rd, monnrd, iyrnrd, &
                       ...
                       iyr1out, monnout, iyrnout, infil, outfilclim, &
                       outfilamip, oldttcalc
!
! Read namelist
!
   read (5,cntlvars)
...
!
   if (infil == ' ') then
      call err_exit ('infil must be specified')
   end if

   if (outfilclim == ' ') then
      call err_exit ('outfilclim must be specified')
   end if
   
   if (outfilamip == ' ') then
      call err_exit ('outfilamip must be specified')
   end if
!

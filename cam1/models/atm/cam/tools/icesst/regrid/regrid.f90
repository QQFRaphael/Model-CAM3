...
!
! Usage: regrid [-a] [-b begin] [-e end] -i icefile [-l] -o outfile -s sstfile [-v]'
!
!----------------------------------------------------------------------------------
   use precision
...
! Local workspace
!
   character*80 :: sstfilein =  ' '     ! input filename for 1x1 SST data
   character*80 :: icefilein =  ' '     ! input filename for 1x1 ICE data
   character*80 :: fileout = ' '        ! input (grid) /output (SST) filename for output data
   character*80 :: arg                  ! cmd line argument
   character*256 :: cmdline             ! input command line
   character*23 :: scattername          ! file name for scatter plot
...
! parse command line arguments, saving them to be written to history attribute
   ...
   if (fileout == ' ') then
      call usage_exit ('No output file specified (e.g. as produced by REGRID.pl')
   end if
!
...
!
   call wrap_nf_open (trim(fileout), NF_WRITE, ncidout)

   call wrap_nf_inq_dimid (ncidout, 'lon', londimidout)
   call wrap_nf_inq_dimlen (ncidout, londimidout, plonout)

   call wrap_nf_inq_dimid (ncidout, 'lat', latdimidout)
   call wrap_nf_inq_dimlen (ncidout, latdimidout, nlatout)
!
...
!
   call wrap_nf_inq_varid (ncidout, 'lon', lonidout)
   call wrap_nf_inq_varid (ncidout, 'lat', latidout)
!
...
!
! Define time dimension on output file
!
...
!
   ...
   call wrap_nf_put_att_text (ncidout, NF_GLOBAL, 'history', totlen, history)
   deallocate (history)
!
...
!   call wrap_nf_copy_att (ncidsstin, timeidin, '_FillValue', ncidout, timeidout)
...
!   call wrap_nf_put_att_real (ncidout, sstidout, '_FillValue', NF_FLOAT, 1, fillvalueout)
   ...
   call wrap_nf_put_att_text (ncidout, iceidout, 'units', len('FRACTION'), 'FRACTION')
...
!
   ret = nf_inq_varid (ncidout, 'nlon', nlonidout)
   if (ret == nf_noerr) then                            ! probably reduced grid
      ret = nf_inq_varid (ncidout, 'rlon', rlonid)
      if (ret == nf_noerr) then
         call wrap_nf_get_var_int (ncidout, nlonidout, nlonout)
         call wrap_nf_get_var_double (ncidout, rlonid, rlon)
      else
         write(6,*)fileout, ' has nlon but not rlon => is screwed up'
         stop 999
      end if
   ...
   call wrap_nf_get_var_double (ncidout, latidout, latout)
...
!
   if (uselandmask) then
      call wrap_nf_inq_varid (ncidout, 'LANDMASK', landmaskidout)
      call wrap_nf_get_var_double (ncidout, landmaskidout, landmaskout)
   else
      landmaskout(:,:) = 0.
   end if
!
...
subroutine usage_exit (arg)
   ...
   if (arg /= ' ') write (6,*) arg
   write (6,*) 'Usage: regrid [-a] [-b begin] [-e end] -i icefile [-l] -o outfile -s sstfile [-v]'
   ...
   write (6,*) '  -o file: input/output netcdf file on which to bin SST and ICE'

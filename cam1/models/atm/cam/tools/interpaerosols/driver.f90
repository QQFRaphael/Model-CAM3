subroutine driver (ncprec, rgridnl)
!------------------------------------------------------------------
! Purpose:
...
!
   ...
   call wrap_put_att_double (ncido, psido, '_FillValue', ncprec, 1, fillvalue)
!
...
!
   ...
   do m = 1, naer
      ...
      call wrap_put_att_double (ncido, varido(m), '_FillValue', ncprec, 1, fillvalue)
   end do
!
...
!
   if (rgridnl /= ' ') then
      call wrap_redef (ncido)
      call wrap_def_var (ncido, 'nlon', NF_INT, 1, latdimido, nlonid)
      dimids(:3) = (/londimido, latdimido, -1/);
      call wrap_def_var (ncido, 'rlon', NF_DOUBLE, 2, dimids, rlonid)
      call wrap_put_att_double (ncido, rlonid, '_FillValue', NF_DOUBLE, 1, fillvalue)
      call wrap_enddef (ncido)
      ...
      call wrap_inq_varid (ncido, 'lon', lonido)
...
!
   ...
   call monthly_to_midmonth (M_ps_cam, 1, nlonout)
!
...
!
      call monthly_to_midmonth (aerosol, nz, nlonout)
...
!
   ...
   if (rgridnl /= ' ') then
      ...
      call wrap_put_var_double (ncido, rlonid, rlonout)
   end if

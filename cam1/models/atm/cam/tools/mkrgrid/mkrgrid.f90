...
!
! $Id: mkrgrid.f90,v 1.2.44.1 2005/02/06 19:46:36 rosinski Exp $
!
  use constants
  use control
  use gridspecs
...
!
  if (reverse) then
    ...
    if (ret == NF_NOERR) then
      ...
      haswnummax = .false.
      if (default_interp == 'fourier') then
         write(6,*)'Cannot do fourier interpolation when wnummax not in input dataset'
         stop 999
      end if
    end if
